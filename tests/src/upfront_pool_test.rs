...
const TICKETS: [ID; 3] = [UPFRONT_BASIC_ID, UPFRONT_MEDIUM_ID, UPFRONT_ADVANCE_ID];
	...
	for i in 0..10 {
		for ticket in TICKETS {
			ExtBuilder::default().build_and_execute(|| {
				...
				init_leave_pool(i, ticket, 1, leave_block);
			});
		}
	}
}
	...
	for i in 0..10 {
		for ticket in TICKETS {
			ExtBuilder::default().build_and_execute(|| {
				...
				init_leave_pool(i, ticket, start_block, leave_block);
			});
		}
	}
}
