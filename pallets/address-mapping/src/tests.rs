use crate::{mock::*, Error, H160Mapping, Config, AddressMapping, pallet, ProofAddressMapping, Id32Mapping, OriginAddressMapping};
...
fn default_into_account_id_works() {
	...
	let origin_address: H160 = PalletAddressMapping::get_or_create_evm_address(account_id);
	assert_eq!(origin_address, address);

	let ALICE = AccountId32::from_str("5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY").unwrap();
	let origin_address = PalletAddressMapping::get_evm_address(ALICE.clone());
...
fn verify_owner_should_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		let address: H160 = H160::from_str("b28049C6EE4F90AE804C70F860e55459E837E84b").unwrap();
		assert_eq!(PalletAddressMapping::verify_bond(ALICE, signature, address.to_fixed_bytes()), true, "verify should works");
	});
}
...
fn bond_should_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		let origin_sender: H160 = PalletAddressMapping::get_or_create_evm_address(sender.clone());
		let signature: [u8; 65] = hex!("20b4f726ffe9333370c64dba5bb50b01e84e1bc8d05b7be0fa8a7c52fcd5c3f46ef44800722a545ad70b8da26fea9cf80fba72a65bb119c7a93e81c3e51edf501b");
		
		let address: H160 = H160::from_str("b28049C6EE4F90AE804C70F860e55459E837E84b").unwrap();
		let origin_address: AccountId32 = ProofAddressMapping::<Test>::into_account_id(address);
		assert_ok!(PalletAddressMapping::bond(Origin::signed(sender.clone()), signature, address, false));
...
fn bond_should_fail() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// incorrect address
		{
			...
			assert_err!(
				PalletAddressMapping::bond(Origin::signed(ALICE), signature, address, true),
		...
		assert_err!(
			PalletAddressMapping::bond(Origin::signed(BOB), signature, address, true),
			<Error<Test>>::SignatureOrAddressNotCorrect
		);
		}
		...
		// insuffcient balance
		{
			...
			assert_err!(PalletAddressMapping::bond(
		...
		// rebond
		{
			...
			assert_err!(
				PalletAddressMapping::bond(Origin::signed(ALICE.clone()), alice_signature, address, true),
				<Error<Test>>::AlreadyBond
			);

			assert_err!(
				PalletAddressMapping::bond(Origin::signed(BOB.clone()), bob_signature, address, true),
				<Error<Test>>::AlreadyBond
			);
		}
	})
...
fn transfer_all_keep_alive_works() {
		...
		let mapping_address = ProofAddressMapping::<Test>::into_account_id(evm_address);
		{
		...
		assert_ok!(PalletAddressMapping::transfer_all(mapping_address.clone(), ALICE.clone(), true));
...
fn transfer_all_allow_death_works() {
		...
		assert_ok!(PalletAddressMapping::transfer_all(mapping_address.clone(), ALICE.clone(), false));
		// evm_address balance should  
		{
...
fn bond_account_balances() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_ok!(PalletAddressMapping::bond(Origin::signed(ALICE.clone()), signature, evm_address, true));
...
fn unbond_works() {
	ExtBuilder::default().build_and_execute(|| {
	...
	
		{
			...
			assert_ok!(PalletAddressMapping::unbond(Origin::signed(ALICE.clone())));
		...
		let origin_address: H160 = PalletAddressMapping::get_or_create_evm_address(ALICE.clone());
...
fn unbond_fail() {
	ExtBuilder::default().build_and_execute(|| {
		run_to_block(10);
		let ALICE = AccountId32::from_str("5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY").unwrap();
		assert_err!(PalletAddressMapping::unbond(Origin::signed(ALICE.clone())), <Error<Test>>::NonbondAccount);
	});
}